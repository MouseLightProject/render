...
const datapath = joinpath(ENV["RENDER_PATH"],"src/render/test/halffilledcube/scratch/data")
...
for i=1:21
  save_tile(datapath, @sprintf("00/000%02d",i), @sprintf("000%02d-halffilledcube",i), "tif", in_tile_jl)
end

yml_file = joinpath(ENV["RENDER_PATH"],"src/render/test/halffilledcube/tilebase.cache.yml")
